...
const Create = () => {
    ...
    const [lableText, setLableText] = useState(t('For free member sub topics is limited to 5'));
    ...
    useEffect(() => {
        ...
        if (sessionStorage.getItem('type') !== 'free') {
            setPaidMember(true);
            setLableText(t('Select number of sub topics'))
        }
    ...
    let addFormFields = () => {
        if (formValues.length < maxSubtopics) {
            ...
            showToast(t('You can only add 5 sub topics'));
        }
    }
            ...
            <div className='dark:bg-black flex-1'>
                ...
                <div className='flex-1 flex items-center justify-center py-10'>
                    ...
                    <form onSubmit={handleSubmit} className="max-w-sm m-auto py-4 no-scrollbar">
                        <p className='text-center font-black text-4xl text-black dark:text-white'>{t('Generate Course')}</p>
                        <p className='text-center font-normal text-black py-4 dark:text-white'>{t('Type the topic on which you want to Generate course.')} <br />
                            {t('Also, you can enter a list of subtopics, which are the specifics you want to learn.')}</p>
                        <div className='py-6'>
                            <div className='mb-6'>
                                <div className="mb-2 block">
                                    <Label className="font-bold text-black dark:text-white" htmlFor="topic1" value={t("Topic")} />
                                </div>
                            ...
                            <div className='mb-6'>
                                <div className="mb-2 block">
                                    <Label className="font-bold text-black dark:text-white" htmlFor="subtopic" value={t("Sub Topic")} />
                                </div>
                            ...
                            <Button type="button" onClick={() => addFormFields()} className='mb-6 items-center justify-center text-center dark:bg-white dark:text-black bg-black text-white font-bold rounded-none w-full enabled:hover:bg-black enabled:focus:bg-black enabled:focus:ring-transparent dark:enabled:hover:bg-white dark:enabled:focus:bg-white dark:enabled:focus:ring-transparent'>{t("Add Sub-Topic")}</Button>

                            {formValues.length > 1 && (
                                <Button type="button" onClick={() => removeFormFields()} className='mb-6 items-center justify-center text-center border-black dark:border-white dark:bg-black dark:text-white bg-white text-black font-bold rounded-none w-full enabled:hover:bg-white enabled:focus:bg-white enabled:focus:ring-transparent dark:enabled:hover:bg-black dark:enabled:focus:bg-black dark:enabled:focus:ring-transparent'>{t("Remove Sub-Topic")}</Button>
                            )}
                            ...
                            <Label className="font-bold text-black dark:text-white" htmlFor="nosubtopic" value={t("Select Course Type")} />
                            <fieldset className="flex max-w-md flex-col gap-4 mt-2">
                                <div className="flex items-center gap-2 px-2 h-11 focus:ring-black focus:border-black border border-black font-normal bg-white rounded-none w-full dark:bg-black dark:border-white dark:text-white">
                                    ...
                                    <Label className='text-black dark:text-white font-bold' htmlFor="textcourse">{t("Theory & Image Course")}</Label>
                                </div>
                                <div onClick={() => showPaidToast()} className="flex items-center gap-2 px-2 h-11 focus:ring-black focus:border-black border border-black font-normal bg-white rounded-none w-full dark:bg-black dark:border-white dark:text-white mb-6">
                                    ...
                                    <Label className='text-black dark:text-white font-bold' htmlFor="videocourse">{t("Video & Theory Course")}</Label>
                                </div>
                            </fieldset>

                            <div className='mb-6'>
                                <div className="mb-2 block">
                                    <Label className="font-bold text-black dark:text-white" htmlFor="code1" value={t("Course Language")} />
                                </div>
                                <Select
